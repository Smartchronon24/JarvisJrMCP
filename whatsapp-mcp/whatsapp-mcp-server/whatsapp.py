...
def safe_parse_date(d_str):
    ...
    try:
        ...
        # handle '2026-08-18 14:21:13 +0530 IST'
        if ' ' in d_str and d_str.count(' ') >= 2:
            d_str = ' '.join(d_str.split(' ')[:2])
        try:
            return datetime.fromisoformat(d_str)
        except:
            return None
...
def list_messages(
    ...
    try:
        ...
        for msg in messages:
            message = Message(
                timestamp=safe_parse_date(msg[0]),
                sender=msg[1],
                chat_name=msg[2],
                content=msg[3],
                is_from_me=msg[4],
...
def get_message_context(message_id: str, before: int = 5, after: int = 5) -> MessageContext:
    ...
    try:
        ...
        target_message = Message(
            timestamp=safe_parse_date(msg_data[0]),
            sender=msg_data[1],
    ...
    try:
        ...
        for chat_data in chats:
            chat = Chat(
                jid=chat_data[0],
                name=chat_data[1],
                last_message_time=safe_parse_date(chat_data[2]) if chat_data[2] else None,
                last_message=chat_data[3],
                last_sender=chat_data[4],
                last_is_from_me=chat_data[5],
                last_read_time=safe_parse_date(chat_data[6]) if chat_data[6] else None,
            )
